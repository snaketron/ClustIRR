...
# 
# save(data_ref_full, file="test_scripts/immuneSIM/immuneSIM_10000seqs.RData")

# load precomputed repertoire
load("test_scripts/immuneSIM/immuneSIM_10000seqs.RData")
...
motifs     <- merge(f_sample,     f_ref, by = "motif", all = TRUE)
motifs_exp <- merge(f_sample_exp, f_ref, by = "motif", all = TRUE)

motifs[is.na(motifs[, "f_ref"]), "f_ref"] <- 0
motifs[is.na(motifs[, "n_ref"]), "n_ref"] <- motifs$n_ref[1]
motifs_exp[is.na(motifs_exp[, "f_ref"]), "f_ref"] <- 0
motifs_exp[is.na(motifs_exp[, "n_ref"]), "n_ref"] <- motifs_exp$n_ref[1]

# compute ove
motifs$ove <- (motifs$f_sample / motifs$n_sample) / 
  (motifs$f_ref / motifs$n_ref)
motifs_exp$ove_exp <- (motifs_exp$f_sample_exp / motifs_exp$n_sample_exp) / 
  (motifs_exp$f_ref / motifs_exp$n_ref)

gt_motifs <- merge(motifs, motifs_exp, 
                   by = c("motif", "f_ref", "n_ref"), all = TRUE)

rm(f_ref, f_ref_exp, f_sample, f_sample_exp, motifs, motifs_exp,
   motif_length)
...
c_v1_i[is.na(c_v1_i)] <- 1e-1
g_v1 <- ggplot(c_v1_i, aes(log(OvE_ClustIRR1_id), log(OvE_ClustIRR1))) +
  geom_point() +
...
c_v2_i[is.na(c_v2_i)] <- 1e-1
g_v2 <- ggplot(c_v2_i, aes(log(OvE_ClustIRR2_id), log(OvE_ClustIRR2))) +
  geom_point() +
  geom_abline(linewidth = 0.5, color = "gray") +
...
c_v3_i[is.na(c_v3_i)] <- 1e-1
g_v3 <- ggplot(c_v3_i, aes(log(OvE_ClustIRR3_id), log(OvE_ClustIRR3))) +
  geom_point() +
  geom_abline(linewidth = 0.5, color = "gray") +
...
c_v1_2[is.na(c_v1_2)] <- 1e-1
g_gl1_gl2 <- ggplot(c_v1_2, aes(log(OvE_ClustIRR1), log(OvE_ClustIRR2))) +
  geom_point() +
...
c_v1_3[is.na(c_v1_3)] <- 1e-1
g_gl1_gl3 <- ggplot(c_v1_3, aes(log(OvE_ClustIRR1), log(OvE_ClustIRR3))) +
  geom_point() +
  #geom_smooth(method = lm , color = "blue", fill = "red") +
  geom_abline(linewidth = 0.5, color = "gray") +
...
c_v2_3[is.na(c_v2_3)] <- 1e-1
g_gl2_gl3 <- ggplot(c_v2_3, aes(log(OvE_ClustIRR2), log(OvE_ClustIRR3))) +
  geom_point() +
  #geom_smooth(method = lm , color = "blue", fill = "red") +
  geom_abline(linewidth = 0.5, color = "gray") +
...
c_v1_2_exp[is.na(c_v1_2_exp)] <- 1e-1
g_gl1_gl2_exp <- ggplot(c_v1_2_exp, aes(log(OvE_ClustIRR1_exp), 
                                        log(OvE_ClustIRR2_exp))) +
  geom_point() +
  #geom_smooth(method = lm , color = "blue", fill = "red") +
  geom_abline(linewidth = 0.5, color = "gray") +
...
c_v1_3_exp[is.na(c_v1_3_exp)] <- 1e-1
g_gl1_gl3_exp <- ggplot(c_v1_3_exp, aes(log(OvE_ClustIRR1_exp), 
                                        log(OvE_ClustIRR3_exp))) +
  geom_point() +
  #geom_smooth(method = lm , color = "blue", fill = "red") +
  geom_abline(linewidth = 0.5, color = "gray") +
...
c_v2_3_exp[is.na(c_v2_3_exp)] <- 1e-1
g_gl2_gl3_exp <- ggplot(c_v2_3_exp, aes(log(OvE_ClustIRR2_exp), 
                                        log(OvE_ClustIRR3_exp))) +
  geom_point() +
  #geom_smooth(method = lm , color = "blue", fill = "red") +
  geom_abline(linewidth = 0.5, color = "gray") +
...
# Compare results only for passed motifs


```{r compare_results_passed}
...
```{r identical_vs_different_passed, fig.width=10, fig.height=10, fig.align='center'}
...
## Check how many motifs are found in different versions

```{r overall_motifs}
# inspect local motif enrichment results
cat("total motif count v1: \n")
cat("identical input: ")
nrow(out_v1_i$clust$CDR3b$local$m)
cat("enriched motifs: ")
nrow(out_v1$clust$CDR3b$local$m)
cat("clonal expansion: ")
nrow(out_v1_exp$clust$CDR3b$local$m)

cat("\n")

cat("total motif count v2: \n")
cat("identical input: ")
nrow(out_v2_i$clust$CDR3b$local$m)
cat("enriched motifs: ")
nrow(out_v2$clust$CDR3b$local$m)
cat("clonal expansion: ")
nrow(out_v2_exp$clust$CDR3b$local$m)

cat("\n")

cat("total motif count v3: \n")
cat("identical input: ")
nrow(out_v3_i$clust$CDR3b$local$m)
cat("enriched motifs: ")
nrow(out_v3$clust$CDR3b$local$m)
cat("clonal expansion: ")
nrow(out_v3_exp$clust$CDR3b$local$m)
...
#### Same algorithm for all versions, so only one per check sufficient


```{r global_motifs}
# inspect which CDR3bs are globally similar
cat("global count identical: ")
nrow(out_v3_i$clust$CDR3b$global)
knitr::kable(out_v3_i$clust$CDR3b$global)

cat("global count motif implanted: ")
nrow(out_v3$clust$CDR3b$global)
knitr::kable(out_v3$clust$CDR3b$global)

cat("global count motif implanted + clone expansion: ")
nrow(out_v3_exp$clust$CDR3b$global)
