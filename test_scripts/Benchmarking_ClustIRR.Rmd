...
- OvE values for found motifs of v1/2 roughly comparable to ground truth

## To be investigated

- p-value of v1 seems to be lacking accuracy compared to v2/3
...
- OvE values for found motifs of v3 partially different than ground truth
  - high value in v3 (i.e. 275.762251) against infinity in gt
  - maybe caused by FET implementation vs formula used for GT
...
f_ref_e_u <- f_ref[f_ref$motif %in% f_sample_e_u$motif, ]
...
rm(f_ref, f_ref_e, f_ref_e_u, f_sample, f_sample_e, f_sample_e_u)
...
r <- res[(res$pass_v1_e==TRUE & !is.na(res$pass_v1_e) ) |
         ...
         c("motif", "pass_v1_e", "pass_v2_e", "pass_v3_e",
           "p_v1_e", "p_value_v2_e", "p_value_v3_e", 
...
For found motifs of version 3 against ground truth, OvE values look different in some values (if **Inf**) -> to be investigated.
