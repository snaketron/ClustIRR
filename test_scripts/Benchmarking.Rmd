...
colnames(e_jan_v1_i)[which(colnames(e_jan_v1_i) == 'OvE')]   <- 'OvE_turboGliph1_identical'
...
c_v1_i <- merge(e_v1, e_v1_i, by = "motif", all=T)
...
c_v2_i <- merge(e_v2, e_v2_i, by = "motif", all=T)
...
c_v3_i <- merge(e_v3, e_v3_i, by = "motif", all=T)
...
c_jan_v1_i <- merge(e_jan_v1, e_jan_v1_i, by = "motif", all=T)
...
c_v1 <- merge(e_v1, e_jan_v1, by = "motif", all=T)
c_v1[is.na(c_v1)] <- -1
...
c_v2 <- merge(e_v2, e_jan_v1, by = "motif", all=T)
c_v2[is.na(c_v2)] <- -1
...
c_v3 <- merge(e_v3, e_jan_v1, by = "motif", all=T)
c_v3[is.na(c_v3)] <- -1
...
c_v1_2 <- merge(e_v1, e_v2, by = "motif", all=T)
...
c_v1_3 <- merge(e_v1, e_v3, by = "motif", all=T)
...
c_v2_3 <- merge(e_v2, e_v3, by = "motif", all=T)
