...
Globally similar sequences *CATSRLEARATYF* (7 times) and *CATSRLEARATYF* 
(13 times) are also marked as locally connected because of motif enrichment.

Global connection between the only two times present sequence 
*CATSEEEFAQYF* is found - but as it is only two times abundant, no local 
enrichment is marked.

```{r check_clustirr_v3}
out3$clust$CDR3b$local$m[out3$clust$CDR3b$local$m$pass,]
out3$clust$CDR3b$global
e3 <- get_edges(out3)
plot_graph(out3, expand_clones = FALSE)
