...
for (version in c(1,2,3)){
    ...
    test_that("data_sample parameter takes only valid input", {
        ...
        df <- data_sample
        df['CDR3a']=42
        expect_error(gliph(data_sample = df, # numerical CDR3a column
                           data_ref = data_ref,
                           version = version,
                           ks = ks,
                           cores = cores,
    ...
    test_that("check_data_sample_and_ref() runs as expected", {
        ...
        expect_error(check_data_sample_and_ref(data_sample_cdr3a,
                                               data_ref))
        # cdr3a column only in data_ref
        expect_error(check_data_sample_and_ref(data_sample,
                                               data_ref_cdr3a))
    ...
    test_that("control_input$global_pairs parameter takes only valid input", {
        control_input_tmp <- control_input
        control_input_tmp$global_pairs <- matrix(data=NA,
                                                 nrow=0,
                                                 ncol=1) # rowcount zero
        expect_error(gliph(data_sample = data_sample,
                           data_ref = data_ref,
                           version = version,
                           ks = ks,
        ...
        expect_error(gliph(data_sample = data_sample,
                           ...
                           cores = cores,
                           control = control_input_tmp))
        control_input_tmp$global_pairs <- matrix(data=17L,
                                                 nrow=1,
                                                 ncol=3) # ≠ 2 columns
        expect_error(gliph(data_sample = data_sample,
                           data_ref = data_ref,
        ...
        control_input_tmp$global_pairs <- matrix(data=as.integer(n),
                                                 nrow=30,
                                                 ncol=2) # wrong index
        expect_error(gliph(data_sample = data_sample,
                           data_ref = data_ref,
