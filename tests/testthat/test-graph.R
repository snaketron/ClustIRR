...
test_that("plot_graph() can handle NA values in s and/or r", {
    ...
    base::suppressWarnings({x <- cluster(s_na, r)})
    p <- plot_graph(x)
    expect_true(sum(is.na(p$x$nodes$clone_count)) == 0)
    base::suppressWarnings({x <- cluster(s_e, r_na)})
    p <- plot_graph(x)
    expect_true(sum(is.na(p$x$nodes$clone_count)) == 0)
    base::suppressWarnings({x <- cluster(s_na, r_na)})
    p <- plot_graph(x)
    expect_true(sum(is.na(p$x$nodes$clone_count)) == 0)
})
...
test_that("plot_graph() can handle NA values in s and/or r with 2 chains", {
    ...
    base::suppressWarnings({x <- cluster(s_na, r_ba)})
    p <- plot_graph(x)
    expect_true(base::sum(base::is.na(p$x$nodes$clone_count)) == 0)
    base::suppressWarnings({x <- cluster(s_ba, r_na)})
    p <- plot_graph(x)
    expect_true(base::sum(base::is.na(p$x$nodes$clone_count)) == 0)
    base::suppressWarnings({x <- cluster(s_na, r_na)})
    p <- plot_graph(x)
