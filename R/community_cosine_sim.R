...
get_norm_cosine_similarity <- function(com) {
    d <- apply(X = com, MARGIN = 2, FUN = sum)
    min_d <- min(d)
    pcom <- apply(X = com, MARGIN = 2, b = 100, min_d = min_d, 
                  FUN = function(x, b, min_d) {
                      return(apply(X = rmultinom(b, size = min_d, prob = x/sum(x)), 
                                   MARGIN = 1, mean))
                  })
    colnames(pcom) <- colnames(com)
    rownames(pcom) <- rownames(com)
    
    
    n <- sqrt(colSums(pcom^2))
    cos_sim <- crossprod(pcom)/(n %o% n)
    colnames(cos_sim) <- colnames(pcom)
    rownames(cos_sim) <- colnames(pcom)
    cos_sim <- reshape2::melt(cos_sim, varnames = c("i", "j"), value.name="CS")
