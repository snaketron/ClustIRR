...
OvE$=\frac{f_{s}}{n_{s}}/\frac{f_{r}}{n_{r}}$
...
We start by loading the needed packages. *knitr*, *ggplot2*, *ggrepel*, and
*visNetwork* are used to display the results.
...
The package provided `CDR3ab` dataset is loaded as an example reference dataset.
...
```{r clustering}
# ClustIRR version 3
clustirr_output_v3 <- cluster_irr(s = s,  
                                  r = r,
                                  version = 3,
                                  ks = 4,
                                  cores = 1,
                                  control = list(
                                    ...
                                    global_pairs = NULL))
# ClustIRR version 2
clustirr_output_v2 <- cluster_irr(s = s,  
                                  r = r,
                                  version = 2,
                                  ks = 4,
                                  cores = 1,
                                  control = list(
                                    ...
                                    global_pairs = NULL))
# ClustIRR version 1
clustirr_output_v1 <- cluster_irr(s = s,  
                                  r = r,
                                  version = 1,
                                  ks = 4,
                                  cores = 1,
...
motif *RQWW* gets found 20 times (`f_s` column). Five other motifs are 
...
(`f_r` > 0), and in this case, by chance, one of them (*GLAA*) was contained 
...
20 times(`f_s` column). The other five motifs are all combinations of the 
...
motifs were marked as enriched. The 20 times inserted motif *RQWW* gets found 
twenty times (`f_s` column). Five other motifs are combinations of the 
...
reference dataset (`mean_f_r`), which is calculated as the mean of repeated 
...
(`mean_f_r`) by *version 1*, which is calculated as the mean of repeated 
bootstrapping, leading in effect to a "happy accident" in which *SYRQ* is found 
...
#### Graph output *(experimental)*

Plotting `clust_irr` objects can be done by calling the `plot_graph()` 
function. Because of potential scaling issues when using larger graphs,
this function is marked as *experimental* for now.

Inside the graph, nodes represent cells. *CDR3b* sequences are colored green, 
*CDR3a* sequences blue.

Groups represent unique sequences, that can be present in form of multiple 
cells / nodes in the graph if these cells are clonally expanded.

Gray edges represent local motif connections, orange edges represent global
connections. 

By hovering over nodes, the sequence is shown. Hovering over edges shows the
shared motif or in case of global connections the shared part of the sequence.


```{r graph_output}
# plot graph of version 1
plot_graph(clustirr_output_v1)

# plot graph of version 2
plot_graph(clustirr_output_v2)

# plot graph of version 3
plot_graph(clustirr_output_v3)
