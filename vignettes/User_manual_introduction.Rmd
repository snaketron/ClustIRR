...
challenges complicate this **clonotype-centric** approach:
...
2.  **Similar TCRs/BCRs recognize the same antigen**: consequently, analyses 
    focused on exact clonotypes may miss biologically meaningful shared immune 
    responses.


This vignette introduces `r Biocpkg("ClustIRR")`, a computational method
that addresses these challenges by: (1) Identifying specificity-associated 
receptor communities through sequence clustering, and (2) Applying Bayesian 
models to quantify differential community abundance across conditions.
...
## **Step 1.** Compute TCR clonotype similarities in a repertoire with `cluster_irr`
...
### **Qualitative** similarity between community abundance vectors with `get_honeycombs`

```{r, fig.width=5, fig.height=5, eval=run}
...
```{r, fig.width=10, fig.height=2.5, eval=run}
wrap_plots(honeycomb, nrow = 1)+
    plot_annotation(tag_levels = 'A')
```

### **Quantitative** similarity between community abundance vectors with `get_cosine_similarity`

In the **heatmap** shown below, we see high cosine similarity (CS) close 
to 1 between repertoires b vs. c, and lower CS values (0.55 -- 0.75) 
between a vs. b/c. This is consistent with the qualitative honeycomb plot 
based analysis.

```{r, fig.width=3, fig.height=2.5, eval=run}
cos_sim <- get_cosine_similarity(com = gcd$community_occupancy_matrix)
cos_sim$g
```

### Summary of communities is provided in the `detect_communities` outputs  

See the object `community_summary`. In this data.frame we provide
...
Node-specific (TCR clonotype-specific) summaries are provided in `node_summary`
...
Community with ID=22 contains 12 TCR clonotypes. These 
are connected based on their CDR3$\alpha$ and CDR3$\beta$ 
sequences.
...
```{r, fig.width=6, fig.height=4, eval=run}
par(mai = c(0,0,0,0))
...
kable(as_data_frame(x = c22$community, what = "vertices")
      [, c("name", "component_id", "CDR3b", "TRBV", "CDR3a", "TRAV")],
      row.names = FALSE)
```
...
-   component id, community id (=22 in this example because this is what
    we selected)
...
### Visualizing the distribution of $\beta$ with `get_beta_violin_ag`
Compare $\beta$s of clonotypes specific for *CMV*, *EBV*, *flu* or *MLANA*?
...
```{r, fig.width=4, fig.height=3, eval=run}
beta_violins$plot
```
...
```{r, fig.width=6, fig.height=2.5, eval=run}
...
```{r, fig.width=7, fig.height=6, eval=run}
...
```{r, echo=FALSE, include=FALSE, eval=run}
