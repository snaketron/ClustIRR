...
* `r`: reference IRR (control/reference sample)
...
To employ a specific global clustering strategy you must set the appropriate
control parameters (`global_smart` and/or `global_pairs`)

*Strategy 1:* if `control$global_smart = FALSE` 

In this case, `r Biocpkg("ClustIRR")` quantifies the dissimilarity between 
pairs of CDR3 sequences using Hamming distances. Two CDR3 sequences with 
Hamming distance $\leq x$ are considered globally similar, where $x$ is the 
user-defined threshold `control$global_max_dist` (default = 1). 
...
and computes a BLOSUM62 score $B$. 

Mean self-similarity score of amino acids in the BLOSUM62 matrix (diagonal 
entries) is about +6. Empirically we know that CDR3 sequences are typically 
short (e.g. 15 amino acids on average). If we multiply $15\cdot6$ we get $B=90$,
i.e. we know that scores $B\approx100$ are associated with pairs of quite 
similar CDR3 sequences. Hence, we transform $B$ to $B'=\max(0, \min(1, B/100))$,
i.e. if $B\geq100 \rightarrow B'=1$ (high degree of global similarity), else if 
$B\leq0 \rightarrow B'=0$ (low degree of global similarity), else $B'=B/100$.
...
P1_A <- base::data.frame(CDR3a = CDR3ab$CDR3a[3001:3100],
                       CDR3b = CDR3ab$CDR3b[3001:3100])
# add clone
P1_A <- rbind(P1_A, P1_A[rep(x = 100, times = 10), ])


P1_B <- base::data.frame(CDR3a = CDR3ab$CDR3a[c(3100, 3101:3200)],
                       CDR3b = CDR3ab$CDR3b[c(3100, 3101:3200)])
# add clone
P1_B <- rbind(P1_B, P1_B[rep(x = 1, times = 20), ])
```

```{r}
P2_A <- base::data.frame(CDR3a = CDR3ab$CDR3a[c(3100, 4001:4100)],
                       CDR3b = CDR3ab$CDR3b[c(3100, 4001:4100)])
# add clone
P2_A <- rbind(P2_A, P2_A[rep(x = 1, times = 5), ])

P2_B <- base::data.frame(CDR3a = CDR3ab$CDR3a[c(3100, 4100:4199)],
                       CDR3b = CDR3ab$CDR3b[c(3100, 4100:4199)])
# add clone
P2_B <- rbind(P2_B, P2_B[rep(x = 1, times = 50), ])
```
...
clust_irrs[[1]] <- cluster_irr(s = P1_A, r = P1_B,
                               version = 2, ks = 4, cores = 1,
                               control = list(global_smart = FALSE,
                                              trim_flank_aa = 3))


clust_irrs[[2]] <- cluster_irr(s = P1_B, r = P1_A,
                               version = 2, ks = 4, cores = 1,
                               control = list(global_smart = FALSE,
                                              trim_flank_aa = 3))


clust_irrs[[3]] <- cluster_irr(s = P2_A, r = P2_B,
                               version = 2, ks = 4, cores = 1,
                               control = list(global_smart = FALSE,
                                              trim_flank_aa = 3))


clust_irrs[[4]] <- cluster_irr(s = P2_A, r = P2_B,
                               version = 2, ks = 4, cores = 1,
