...
We start by loading the needed packages. *knitr* and *visNetwork* will be used
...
The extracted edges contain informations about local and global connections
between cdr3 sequences as well as the shared motif and source chain.
...
To extract the edges as *igraph* object for analysis or visualization with 
other packages, the `get_graph()` function can be called on our output file of 
class `clust_irr`.

For more information about *igraph*, consult the packages manual.

```{r igraph_output}
# extract edges
ig <- get_graph(clustirr_output)
