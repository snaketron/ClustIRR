...
context of the major histocompatibility complex (MHC). B cell receptors (BCRs),
on the other hand, are protein complexes expressed on B cells that bind 
directly to antigens. In the following we will refer to BCRs and TCRs using
the general term immune receptors (IRs).

Owing to VDJ recombination, virtually every B/T cell expresses its own unique 
IR, and hence has a unique specificity. Understanding the relationship between 
the IR sequence and antigens is essential for the development of e.g. cancer 
...
enable the analysis of entire repertoires of IRs in a single experiment, but 
the currently available methods for 1) organizing clusters of IR sequences 
...
time points, have crucial drawbacks or are not available.
