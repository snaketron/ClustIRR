...
```
In this graph, enriched motifs *LEAR* and *LLEA* as well as the global connection
*CAS-PLEARGYTF* between cells of the sample are visualized. <br>
...
Shared motifs between cells are represented by gray `local edges`, cells sharing 
motifs as well as global sequences are connected by red `local & global edges`
edges. <br>
...
plot_graph(out3)
```
In this graph, cells that share local motifs and global sequences are connected with
red edges. Blue nodes signify cells that share local motifs and/or global 
