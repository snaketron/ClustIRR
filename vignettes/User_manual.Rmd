...
Two different $\delta$ parameters are available.
${\delta}_{\beta}$ is the difference between the different samples $\beta$ parameter
for each community. It can be interpreted as the effect of each community, 
ranging from $-\infty$ to $+\infty$.

```{r, fig.width=6, fig.height=4}
ggplot(data = d$posterior_summary$delta_b)+
  ...
  ylab(label = expression(delta[beta]))+
  scale_x_continuous(expand = c(0,0))
```

${\delta}_p$ is the difference between the different samples
regenerated multinomial probability $p$ for each community. It can be 
interpreted as the effect of each community, ranging from $-1$ to $+1$.

```{r, fig.width=6, fig.height=4}
ggplot(data = d$posterior_summary$delta_p)+
  facet_wrap(facets = ~contrast, ncol = 1)+
  geom_errorbar(aes(x = community, y = mean, ymin = L95, ymax = H95), 
                col = "darkgray", width =0)+
  geom_point(aes(x = community, y = mean), size = 0.5)+
...
ggplot(data = d$posterior_summary$delta_b)+
  ...
  xlab(label = expression(delta[beta]))+
  theme_bw(base_size = 10)
```
...
`posterior_summary$delta_b`. of the output. We see clear differences
...
```{r, fig.width=6, fig.height=7}
delta <- d$posterior_summary$delta_b
...
```{r, fig.width=6, fig.height=7}
delta <- d$posterior_summary$delta_p
delta <- delta[delta$contrast %in% c("a-b", "a-c", "b-c"), ]
