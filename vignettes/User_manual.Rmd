...
both chains (CDR3$\alpha\beta$). The two sets represent two repertoires, and
each element in the sets represents a T-cell:
...
For global clustering ClustIRR quantifies the dissimilarity between pairs of 
CDR3 sequences by using Hamming distances. Two CDR3 sequences with Hamming 
distance $\leq x$ are considered globally similar, where $x$ is the user-defined 
...
# Case study 1: simple TCR repertoire analysis with ClustIRR
...
One of the main outputs of ClustIRR is an undirected graph. Each vertex is a
T-cell (row in repertoire `s`), and we draw an edge between two vertices if
a) they are globally similar; or b) if they share an enriched motif. Vertices
that are not connected by an edge to any other vertex are not shown. 
...
# Case study 2: analysing TCR repertoire with large expanded clone
...
that belong to the T-cells of the expanded clone. Each pair of CDR3 sequences 
in the clone are globally similar. Meanwhile, the CDR3 sequences 
*CAS**S**PLEARGYTF* and *CAS**R**PLEARGYTF* differ by one amino acid at 
position 4.
...
larger than n=10. If the clonal expansion contains n=100 T-cells then ClustIRR
would draw a completely connected component with 100 vertices and about 5000
edges between these vertices, which may lead to overplotting. 
...
```{r}
# plot the clust_irr object
plot_graph(clust_irr = clustirr_output, expand_clones = FALSE)
```

By hovering or clicking on the big vertex we get a summary of all edges 
between the members of the clone. 
...
# Case study 4: analysing TCR repertoires of $\gamma\delta$ T-cells
