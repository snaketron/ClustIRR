...
`r Biocpkg("ClustIRR")` does not perform any kind of clonotyping, the sequences
supplied by the user are not collapsed before clustering. To integrate clone
size information in the analysis, an optional `clone_size` column can be added
to the input table.
...
# s <- data.frame(CDR3b = CDR3ab$CDR3b[1:500])
...
user-defined threshold `control$global_max_hdist` (default = 1). 
...
clone <- data.frame(CDR3b = rep("CATSRPDGLAQYF", times = 20))
...
P2_B <- data.frame(CDR3a = CDR3ab$CDR3a[c(3100, 4100:4199)],
