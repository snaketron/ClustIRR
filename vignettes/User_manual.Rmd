...
which detects communities of immune receptors with similar specificity;
and employs Bayesian models to quantify differential community occupancy
between IRRs from different biological conditions (e.g. before and after
cancer treatment).
...
-   **amino acid sequence/s** of the complementarity determining regions
    3 from one or both chains (e.g. CDR3$\alpha$ and CDR3$\beta$ from
    $\alpha\beta$ TCRs).
-   **clone size** = the frequency of cells that belong to the clone
...
CDR3$\beta$ alignment scores (**notice:** in the next, I will use
TCR$\alpha\beta$ as an example. However, this approach can also be used
to compare TCR$\gamma\delta$ or BCR*IgH-IgL* clones): \begin{align}
    \omega(i,j)=\dfrac{{\bar{\omega}}^\alpha + {\bar{\omega}}^\beta}{2}
    \qquad\text{or}\qquad
    \omega(i,j)=\dfrac{{\bar{\omega}}^\alpha_c + {\bar{\omega}}^\beta_c}{2},
\end{align} where $\bar{\omega}^\alpha$ and $\bar{\omega}^\beta$ are the
...
**Important note**: If the data contains CDR3 sequences from only one
chain, such as CDR3$\beta$, then $\omega(i,j)$ is defined as
\begin{align}
...
-   `max_len` and `max_clen`: length of the longer CDR3 sequence and
    CDR3 core sequence in the pair
-   `weight`: BLOSUM62 score of the **complete** CDR3 alignment
-   `cweight`: BLOSUM62 score of CDR3 **cores**
-   `nweight`: normalized `weight` by `max_len`
-   `ncweight`: normalized `cweight` and `max_clen`
...
edges between TCR clones from different IRRs are computed using the same
...
`get_graph_dummy`, which allows the user to skip step. 1 of the
algorithm. The inputs of this function is IRR `s` (same as before) and a
data frame containing the (symbolic) edge list. This enables seamless
integration of `r Biocpkg("ClustIRR")` with computational pipelines that
employ other IRR clustering approaches, such as gliph, TCRdist3, GIANA,
and others.
...
-   `weight_type`: "ncweight" (default) or "nweight"
...
Also see `community_summary` if you want to see the frequency of clones
or cells in each community (per IRR), their mean edge weights, etc.
...
ggplot(data = gcd$community_summary)+
  geom_point(aes(x = w_CDR3a_mean, y = w_CDR3b_mean, size = cells_n), shape=21)+
