...
# Case study 5: analysis of more than two TCR repertoires

Let's create the input data. We create 4 repertoires: 

  * repertoire `A0`: 500 CDR3$\beta$ and CDR3$\alpha$ sequence pairs. 
  * repertoire `A1`: 500 CDR3$\beta$ and CDR3$\alpha$ sequence pairs. 
  * repertoire `B0`: 500 CDR3$\beta$ and CDR3$\alpha$ sequence pairs. 
  * repertoire `B1`: 500 CDR3$\beta$ and CDR3$\alpha$ sequence pairs. 
  
Imagine we have two cancer patients, `A` and `B`. From each patient we have 
two samples, one before (0) and one after (1) treatment:
...
```{r}
A0 <- base::data.frame(CDR3a = CDR3ab$CDR3a[3001:3100],
                       CDR3b = CDR3ab$CDR3b[3001:3100])
# add clone
A0 <- rbind(A0, A0[rep(x = 100, times = 10), ])


A1 <- base::data.frame(CDR3a = CDR3ab$CDR3a[c(3100, 3101:3200)],
                       CDR3b = CDR3ab$CDR3b[c(3100, 3101:3200)])
# add clone
A1 <- rbind(A1, A1[rep(x = 1, times = 20), ])
```

```{r}
B0 <- base::data.frame(CDR3a = CDR3ab$CDR3a[c(3100, 4001:4100)],
                       CDR3b = CDR3ab$CDR3b[c(3100, 4001:4100)])
# add clone
B0 <- rbind(B0, B0[rep(x = 1, times = 5), ])

B1 <- base::data.frame(CDR3a = CDR3ab$CDR3a[c(3100, 4100:4199)],
                       CDR3b = CDR3ab$CDR3b[c(3100, 4100:4199)])
# add clone
B1 <- rbind(B1, B1[rep(x = 1, times = 50), ])
```


```{r}
clust_irrs <- vector(mode = "list", length = 4)
names(clust_irrs) <- c("A0", "A1", "B0", "B1")

clust_irrs[[1]] <- cluster_irr(s = A0, r = A1,
                               version = 2, ks = 4, cores = 1,
                               control = list(trim_flank_aa = 3))


clust_irrs[[2]] <- cluster_irr(s = A1, r = A0,
                               version = 2, ks = 4, cores = 1,
                               control = list(trim_flank_aa = 3))


clust_irrs[[3]] <- cluster_irr(s = B0, r = B1,
                               version = 2, ks = 4, cores = 1,
                               control = list(trim_flank_aa = 3))


clust_irrs[[4]] <- cluster_irr(s = B1, r = B0,
                               version = 2, ks = 4, cores = 1,
                               control = list(trim_flank_aa = 3))
```
