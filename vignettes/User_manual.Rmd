...
A function to plot a graph directly is planned to be added in future package 
versions.
...
To demonstrate the `r Biocpkg("ClustIRR")` workflow, a short case study is 
conducted. The newest *version 3* will be used. For a comparison of the 
...
```{r clustering}
# perform clustering
clustirr_output <- cluster_irr(data_sample = data_sample,  
                               data_ref = CD8,
                               version = 3,
                               ks = 4,
                               cores = 1,
...
shorter than 6 aa completely. As long as our sample dataset is not cut, this 
should be fine.
...
20 times (to be seen in the column `f_sample`). Five other motifs are 
...
sequences *CATSRAAKPDGLAALETQYF* and *CATSRAAKPDGLAALSTQYF* are found. Some of 
them are found in both sequences (for example *AAKP*, found 30 times), some of 
them only in one of the two (for example *AALE*, found 15 times). Notice that 
motifs can also present in the reference dataset as parts of other sequences 
(`f_ref` > 0). In our case, for example, the enriched motif *GLAA* was 
...
expanded sequences are guaranteed to be detected by the algorithm 
(*version 3*).
...
To extract the edges for a visualisation, the `get_edges()` function can be 
