...
output: 
  prettydoc::html_pretty:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{Introduction to gliphR}
...
**gliphR** performs clustering of T-cell receptors (TCRs) to find 
groups of TCRs with similar specificity. Quantitative comparison of TCR cluster
structure between biological conditions. Includes a low memory mode to 
reduce memory usage for large input files.
...
a minimum for user convenience. The first edition of **gliphR** implements the 
...
gliphR can be installed using the BiocManager package.
...
To run **gliphR** with default parameters, we provide the columns **CDR3b**,
